...
public class Facility {
	...
	public boolean addPatient(CurrentPatient patient) {
		for (CurrentPatient x : patientList) {
			if (x.equals(patient))
				return false;
		}
	...
	public CurrentPatient lookupByUHID(UHealthID patientID) {
		for (CurrentPatient x : this.patientList) {
	...
	public ArrayList<CurrentPatient> getInactivePatients(GregorianCalendar date) {
		ArrayList<CurrentPatient> List = new ArrayList<CurrentPatient>();
		for (CurrentPatient patients : this.patientList) {
	...
	public ArrayList<Integer> getPhysicianList() {
		...
		for (CurrentPatient x : this.patientList) {
			int phy = x.getPhysician();
			if (!(phyList.contains(phy))) {
				phyList.add(phy);
			}
